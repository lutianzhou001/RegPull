...
df_transfers = pd.read_csv("../../data/token_transfers_features.csv").drop_duplicates(subset=['token_address'])
...
df['late_creation'] = df['late_creation'] * df['late_creation_transfers']
df = df.drop(["late_creation_transfers", 'inactive_transfers'], axis=1)
...
pd.DataFrame(label_list, columns=["token_address", "pool_address", "label", "type"]).to_csv("labeled_list.csv", index=False)