...
Uniswap_token_abi = '0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984'
f = open(f'../data/Contract_abis/{Uniswap_token_abi}.json')  # obrim json abis
abi = json.load(f)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=20) as exec:
    for i, token_addr in enumerate(token_info['token_address'].tolist()):
        if token_addr not in token_transfers_list and (token_addr not in healthy_tokens and token_addr.lower() not in healthy_tokens) and token_addr.lower() not in huge_tokens:
